...
这是寻找原始材料时的归档线索，不是克隆后的运行路径。公开仓是对原目录的精简和便携化整理，不是原目录的完整镜像。

| 原始归档内容 | 公开仓库中的处理 | 对克隆者的影响 |
| --- | --- | --- |
| `collect_images.py`、`collect_new.py`、`collect_only.py`、`collect_simple.py` | 以同名主程序上传，已整理参数、原始帧保存、文件命名和写盘检查 | 可直接阅读和运行，但不是与历史文件逐字节相同的副本 |
| `turn.py`、`turn1.py`、`turn2.py` | 以同名实验入口上传，深度点击和坐标说明已整理 | RGB-only 与 RGB+深度实验主线都保留 |
| 原 README 和笔记 | 重新整理为当前 `README.md` 和详细 `note.md` | 以公开仓文档描述的行为为准 |
| `dataset/`、`train*/`、`real*/`、`fake*/`、`olddataset/` | 未上传 | 历史 RGB 图片、分类目录和少量 YOLO 标注不在 Git 中 |
| 历史 ZIP 和其他重复包 | 未上传 | 它们是数据副本或交付包，不是采集程序的运行依赖 |
| 无历史对应文件 | 新增 `capture_utils.py`、`tools/build_manifest.py`、依赖、数据政策及法律/第三方说明 | 用于公开交接、防覆盖保存和数据审核 |

未上传的历史数据只有在完成权利、隐私、去重和按拍摄 session 切分审核后，才会考虑放到 Hugging Face Datasets 等独立数据托管平台。目前没有可用的公开数据集地址。

## 只克隆仓库后还缺什么

Git 克隆包含完整的公开采集程序，但不包含相机硬件、系统驱动、采集结果或团队的数据验收规则。

| 目标 | 克隆后仍需提供 |
| --- | --- |
| 运行普通 RGB 采集 | Intel RealSense 相机、匹配的 librealsense/固件、USB 访问权限、`pyrealsense2`、NumPy、带 GUI 的 OpenCV、桌面会话与可写输出目录 |
| 运行 `turn1.py` / `turn2.py` | 上述条件，以及能同时提供彩色流和深度流的设备/profile |
| 开始正式批量采集 | 算法组给出的类别定义、边界样本、场景覆盖、session 命名和验收规则，以及不在 Git 仓库内的受控数据存储 |
| 将采集结果用于训练 | 独立的标注工具/规范、去重与 train/val/test 切分流程、数据卡、训练代码和模型仓库；这些不是本采集仓库的运行部分 |

本工具不需要 ROS、CAN、已有训练数据或神经网络模型才能完成采集。`turn1.py` / `turn2.py` 的坐标旋转也不等于已完成机器人外参标定。

## 代码、模型与数据的存储方式

- 代码、文档和数据治理工具使用普通 Git 跟踪，当前没有使用 Git LFS。
- 本仓库只采集图片，不执行模型推理，因此没有必须另行下载的 `.pt`、`.onnx` 或模型 Release。
- 采集出的图片、标注、manifest 和审核记录应保存在仓库外的受控数据目录；不要将 `--output-dir` 指向源码仓内。
- 如果未来公开数据，会通过独立数据集卡、版本和校验和与代码分开发布；当前不应假设历史数据可下载。
