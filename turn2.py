"""RGB/depth click experiment adding 3D distance and optical-axis offset."""
...
def main() -> int:
    ...
    args = parse_args()
    manual_angles = list(args.angles) if args.angles is not None else prompt_angles()
    ...
    try:
        ...
        print("Click=measure | a=angles | r/f=save raw RGB | q=quit")
        ...
        while True:
            ...
            cv2.putText(
                preview,
                f"Angles: {manual_angles} | click=measure | a=reset",
                (10, 30),
            ...
            if click_point is not None:
                ...
                if camera_point is None:
                    ...
                    distance_3d = math.sqrt(x_cam**2 + y_cam**2 + z_cam**2)
                    optical_axis_offset = math.sqrt(x_cam**2 + y_cam**2)
                    print(
                        "Camera frame [m]: "
                    ...
                    print(f"3D range: {distance_3d:.3f} m")
                    print(f"Offset from optical axis: {optical_axis_offset:.3f} m")
                    cv2.circle(preview, (x, y), 4, (0, 0, 255), -1)
                    label_x = x + 10 if x < args.width - 140 else max(0, x - 140)
                    label_y = y + 20 if y < args.height - 30 else max(15, y - 15)
                    cv2.putText(
                        preview,
                        f"range={distance_3d:.2f}m",
                        (label_x, label_y),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.5,
                        (0, 0, 255),
